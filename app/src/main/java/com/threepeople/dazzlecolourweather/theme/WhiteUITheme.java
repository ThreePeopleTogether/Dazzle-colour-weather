 ...
 * 白色UI主题.
 */
public class WhiteUITheme implements UITheme {
    ...
    public WhiteUITheme(Context context) {
        oneDp = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1.0f, context.getResources().getDisplayMetrics());
        iconColor   = Color.rgb(42, 42, 42);// #2A2A2A
        textColor   = Color.rgb(42, 42, 42);// #2A2A2A
        stressColor = Color.rgb(129, 190, 247);// #81BEF7
        ignoreColor = Color.rgb(188, 188, 188);// #BCBCBC
        themeColor  = Color.rgb(242, 242, 250);// F2F2FA
    }
    ...
    public Drawable getInputCityBackground() {
        if (inputCityBackground == null) {
            inputCityBackground = XMLDrawable.getCircleCornerShape(Color.WHITE, 15 * oneDp);
        }
